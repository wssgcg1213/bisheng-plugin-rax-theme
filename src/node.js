...
module.exports = function(markdownData) {
  ...
  const filename = markdownData.meta && markdownData.meta.filename
      && markdownData.meta.filename.replace(/^_tmp\/(internal|external)Docs\/(zh-Hans|)\//, '')
        .replace(/\.md$/i, '');
  const title = markdownData.content && markdownData.content[1] &&
  markdownData.content[1][0] === 'h1' && markdownData.content[1][1] || '';
  ...
  if (!markdownData.title && markdownData.url === '/theme/contribute') {
    markdownData.title = '如何贡献 Rax 主题';
  }
  return markdownData;
};
